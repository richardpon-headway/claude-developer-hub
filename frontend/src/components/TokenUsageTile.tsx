...
export function TokenUsageTile() {
  ...
  const todayOutput = query.data.today_output;
  const todayMessages = query.data.today_messages;
      ...
      <div className="mt-2 flex items-baseline gap-4">
        <div>
          <div className="text-2xl font-semibold text-zinc-100">
            {formatNumber(todayOutput)}
          </div>
          <div className="text-xs text-zinc-500">output</div>
        </div>
        <div className="text-xs text-zinc-500">
          {formatNumber(todayMessages)} message{todayMessages === 1 ? "" : "s"}
        </div>
      </div>
      {top.length > 0 && (
        <>
